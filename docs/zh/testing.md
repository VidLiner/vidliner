...
当前规模：**397 个测试**，全部通过；`ruff` 与 `ty` 均无输出。
...
tests/
  ...
  fixtures/              只为测试而存在的 backend：一个"删掉对象"的生成器，
                         以及生产防护用的声明式标记类
  unit/                  规范序列化与身份、领域模型、引擎、存储、质量、标注、
                         生成后验证、生产防护、文档一致性、架构规则
  contract/              通过协议逐个验证每个 backend
  pipeline/              端到端、导出、恢复与缓存、验证回路、CLI
```
...
| 验证回路 | 生成图里没有的对象给出 `found=False` 与 `OBJECT_NOT_FOUND`，而不是节点失败；导出的标签来自重建后的 mask |
| 生产防护 | 演示栈在预检与导出两处被拒，四项关键能力一次列全，且以 manifest 为唯一依据 |
| 文档 | 中英文两侧的 reason code 清单与能力表都与代码一致 |
...
6. 如果新增了 reason code 或能力名，`tests/unit/test_docs_consistency.py` 会在中英文两侧都列出它之前一直失败。镜像封闭枚举的文档要么由它生成，要么与它比对；这里是比对，因为清单周围的说明文字值得保留。
